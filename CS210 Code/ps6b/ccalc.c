...
writeResults(long long result) {
  ...
  } //write the value of SUM_POSITIVE to standard output in its binary form
  if (write(1, &SUM_NEGATIVE, sizeof(SUM_NEGATIVE)) != sizeof(SUM_NEGATIVE)) {
	fprintf(stderr, "ERROR: write SUM_NEGATIVE failed\n");
  } //write the value of SUM_NEGATIVE to standard output in its binary form
  long long length = ((void *)&CALC_DATA_END)-((void *)&CALC_DATA_BEGIN); //calculate the difference in bytes  
      if (write(1, (void *)&CALC_DATA_BEGIN, length) !=length) {
      fprintf(stderr, "ERROR: write data length failed\n");
      } //write the area of memory between &CALC_DATA_BEGIN and & CALC_DATA_END to standard out as its raw binary values
